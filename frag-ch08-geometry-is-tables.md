...
There's a practical consequence too: R's `polygon()` function can only fill one colour at a time, which means sp had to draw every single object as a separate function call. The entire architecture of spatial plotting in R was shaped by a renderer limitation, not a data model choice. (Manifold GIS, by contrast, stores points, lines, and areas in a single "drawing" — all three topological types coexisting in one layer, breakable by query rather than by file format. Almost nobody else does this.)
